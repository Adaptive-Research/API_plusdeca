...
if ( isset($_POST['Submit']) ) 
{
  ...
  if (isset($_POST['token']))
  {
    ...
    if ($pos >= 0)
    {
      ...
      if ( $num_rows > 0 )        
      {
        $sql2 = "select * from utilisateur_payant where idutilisateur = ".$idUser ;
        ...
        if ( $num_rows2 > 0 )
        {
          ...
          {
            $objK = new UtilisateurPayant_API ;
            $objK->id = $row['id'] ;
            $objK->idUtilisateur = $row['idutilisateur'] ;
            $objK->idFonction = $row['idfonction'] ;
            $objK->DateFin = $row['date_fin'] ;
