  ...
  if (isset($_POST['token']))
  {
    ...
    if ($pos >= 0)
    {
      ...
      if ( $num_rows > 0 )
      {
        $idGroupe = "" ;
        if ($_POST['idGroup'] != "")
        $idGroupe = str_replace("'","''",$_POST['idGroup']) ;
            ...
            if ( $num_rows2 > 0 )
            {
              ...
              if ( $num_rows3 > 0 )
              {
                  while($row = pg_fetch_assoc($result3))
                  {
                    ...
                    if($idCreator !== $idUser){
                      $content = "".$row['prenom']." ".$row['nom']." a quitté votre groupe ".$NomGroupe."";
                      
                      $sql4 = "delete from groupe_utilisateur where idgroupe = ".$idGroupe. " and idutilisateur = ".$idUser;
                      $result4 = pg_query($conn, $sql4);
                      if (isset($_POST['debug']))
                        echo $sql4."\n" ;
              
                      if($result4 !== false) {  
                        echo "OK Unsubscription done" ;
                        ...
                        echo "ERROR: Unsubscription not done" ;
                      }
                      
                    }
                    else {
                      echo "ERROR: You can't unsubscribe to your own group, cause you're the creator" ;
                    }
                  }
              }
            }else {
              echo "ERROR: You're not a member of this group" ;
            }
