    ...
    if ($pos >= 0)
    {
      ...
      if ( $num_rows > 0 )
      {
        if (isset($_POST['idAncestor']))
          $sql = "update formations set isvalidated = 1 where idancestor = ".$_POST['idAncestor']." and numversion = ".$_POST['NumVersion']   ;
        else 
          $sql = "update formations set isvalidated = 1 where id = ".$_POST['id']   ;
        if (isset($_POST['debug']))
          echo $sql."\n" ;
        $result = pg_query($conn, $sql);
        if($result !== false)
          ...
          echo "ERROR: Formation not validated" ;
      }
      else
