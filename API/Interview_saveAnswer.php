...
if ( isset($_POST['Submit']) )
{
  ...
  if (isset($_POST['token']))
  {
    ...
    if ($pos >= 0)
    {
      ...
      if ( $num_rows > 0 )
      {
        $sql = "update interview_reponses set iscurrent = 0 where idutilisateur = ".$idUser." and idquestion = '".$_POST['idQuestion']."'"   ;
        if (isset($_POST['debug']))
          echo $sql."\n" ;
        $result = pg_query($conn, $sql);

        $rep = str_replace("'","''",$_POST['Reponse']) ;

        $sql = "insert into interview_reponses ( idutilisateur,  idinterview,  idquestion, reponse ) values ('".$idUser ;
        $sql = $sql."','".$_POST['idInterview']."','".$_POST['idQuestion']."','".$rep."')" ;
