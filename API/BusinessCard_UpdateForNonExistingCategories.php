...
if ( isset($_POST['Submit']) ) // Soit c'est la 1ère sauvegarde, soit c'est au moins une sauvegarde supplémentaire
{
  ...
  if (isset($_POST['token']))
  {
    ...
    if ($pos >= 0)
    {
      ...
      if ( $num_rows > 0 )
      {
        $sql = "update businesscard_classement set idcategorie = 0 where idutilisateur = ".$idUser." and idcategorie not in (select id from businesscard_categories where idutilisateur = ".$idUser." )"    ;
        if (isset($_POST['debug']))
          echo $sql."\n" ;
        ...
        if($result !== false){
          ...
          echo "ERROR: Categorie not deleted" ;
      }
      else
