...
if ( isset($_POST['Submit']) ) // Soit c'est la 1ère sauvegarde, soit c'est au moins une sauvegarde supplémentaire
{
  ...
  if (isset($_POST['token']))
  {
    ...
    if ($pos >= 0)
    {
      ...
      if ( $num_rows > 0 )
      {      
        $sql = "update entreprise_etablissement set iscurrent = 0 where id  = ".$_POST['id']   ;
        if (isset($_POST['debug']))
          echo $sql."\n" ;        
        $result = pg_query($conn, $sql);

        $sql = "update entreprise_utilisateur set iscurrent = 0 where identreprise  = ".$_POST['id']   ;
        if (isset($_POST['debug']))
          echo $sql."\n" ;        
        $result = pg_query($conn, $sql);
        ...
        if($result !== false)
          ...
          echo "ERROR: Company not deleted" ;
