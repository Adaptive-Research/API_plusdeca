      ...
      if ( $num_rows > 0 )
      {
        ...
        if ( $num_rows > 0 )
        {
          while($row = pg_fetch_assoc($result))
          {
            ...
            $objK->id = $row['id'] ;
            $objK->isread = $row['isread'] ;
            $objK->idutilisateur = $row['idutilisateur'] ;
            $objK->notification_content = $row['notification_content'] ;
            $objK->idtype_notification = $row['idtype_notification'] ;
          ...
          echo "ERROR: no Notification for this user in database" ;
      }
      else
